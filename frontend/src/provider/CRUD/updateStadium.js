...
export default function UpdateStadium() {
  ...
  useEffect(()=>{
    ...
    SearchCourt(id).then((res)=>{
      console.log(res)
      if(res[0].ball_type_id.includes(",")){
        setSport(res[0].ball_type_id.split(",").map((sportId)=>{
          return Object.keys(sportType).find(key => {
            return Object.is(sportId,sportType[key])});
        }))
      }
      else{
